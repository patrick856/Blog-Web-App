...
app.get("/", (req, res) => {
  ...
  res.render("index.ejs", { currentYear: new Date().getFullYear() });
});

app.post("/submit", (req, res) => {
  const randomnumber1 = Math.floor(Math.random() * adj.length);
  const randomnumber2 = Math.floor(Math.random() * noun.length);
  const bandname = `${adj[randomnumber1]} ${noun[randomnumber2]}`;
  res.render("index.ejs", { bandname: bandname, currentYear: new Date().getFullYear() });
});
